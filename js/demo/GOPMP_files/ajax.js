...
function Ajax(recvType, statusId){
	...
	aj.processHandle = function(){
		if(aj.statusId) aj.statusId.style.display = '';
		if(aj.XMLHttpRequest.readyState == 1 && aj.statusId){
			...
			if(aj.XMLHttpRequest.status == 200){
				for(k in Ajaxs) if(Ajaxs[k] == aj.targetUrl) Ajaxs[k] = null;
				if(aj.statusId){
					aj.statusId.innerHTML = xml_http_data_in_processed;
					aj.statusId.style.display = 'none';
				}
				if(aj.recvType == 'HTML') aj.resultHandle(aj.XMLHttpRequest.responseText, aj);
				else if(aj.recvType == 'XML') aj.resultHandle(aj.XMLHttpRequest.responseXML.lastChild.firstChild.nodeValue, aj);
				else if(aj.recvType == 'JSON') aj.resultHandle(eval('('+aj.XMLHttpRequest.responseText+')'), aj);
			}else{
	...
	aj.get = function(targetUrl, resultHandle){
		...
		else arraypush(Ajaxs, targetUrl);
